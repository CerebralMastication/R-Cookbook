...
It's worth noting that the `summarize` function will silently remove the last of the grouping variables from the `group_by`. After a summarize statement is completed the result will have one, and only one, record per grouping. So after the `summarize` is completed, the last grouping variable is dropped automatically from the grouping. The last grouping field is still in the data, but it's not used for groupings. This can be surprising the first time you observe it in action. 
